...
or it consists of a node, which is its root.\footnote{Yet
...
the tree contains and the height of the tree.
...
in its root, the key $j$ in the root of its left subtree,
and the key $k$ in the root of its right subtree.
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.36]{images/zig-and-zag.png}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.26]{images/dbl-rotation.png}
\end{center}
\index{tree!rebalance}\index{searchtree!rebalance}
\index{AVL tree!rebalance}\index{balance!AVL rebalance}
...
When we apply the \textsf{zag} operator to
...
\Exercise Prove that \textsf{(ins $x$ $a$ nil)} delivers a tree that is
ordered and balanced.

\Exercise Prove that \textsf{(ins $x$ $a$ $s$)}
delivers an ordered, balanced tree if (height $s$) = 1.

\Exercise Define \textsf{rot-} \label{defun:rot-}
