...
of a tree is the maximum number of keys
...
\label{empty-tree-representation}The empty list (nil) will represent the
...
represents a tree.
It has a key, a left subtree, and a right subtree.
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.7]{images/unbalanced-trees.png}
\end{center}
\index{balance!AVL tree}\index{AVL tree!balance}\index{search tree!balance}
...
\label{balance-defun}The predicate ``balp'' expresses this notion formally
...
case the hook operator encounters a key that is the same
...
as the second operand in the invocation of hook.
...
the hook operator, by itself, is adequate for
inserting new keys.
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.29]{images/ht2-or-more-ex.png}
\end{center}
\index{tree!balance}\index{searchtree!balance}
\index{AVL tree!balance}\index{balance!AVL tree}
...
require no further attention.}\index{tree!balance}\index{searchtree!balance}\index{AVL tree!balance}\index{balance!AVL tree}\
\label{cases:ht-n+2}\begin{quote}
\begin{enumerate}
\item case \emph{LL}: $x < j < k$ (refer to Figure \ref{fig:trees-of-ht-n+2-expanded})
...
Figure \ref{fig:trees-of-ht-n+2-expanded} (page \pageref{fig:trees-of-ht-n+2-expanded})
that we use to analyze the case.
...
Case \emph{RR} is a mirror reflection of
case \emph{LL} and deals with the situation when the new key $x$
is greater than $i$ ($x > i > k$).
Case \emph{RL} (which mirrors case \emph{LR})
...
are ``ìnside cases'' because the problematic subtrees
...
In other words, clockwise rotation doesn't help,
...
The diagram makes it clear that we can apply zag
...
to do a zag rotation if \emph{jRx} were empty.}
When we apply the zag operator to
the left subtree of \emph{LRin},
we get the following tree,
with the algebraic formula [$y$ $p$ \emph{yL} \emph{yR}]
for subtree \emph{jRx}.\footnote{The formula uses $p$ to denote
...
of the insertion operator, ins, is complete, and
Figure \ref{defun:ins} (page \pageref{defun:ins})
displays the formal definition of ins.
...
Check it out online or a in textbook on algorithms.
...
\index{AVL tree!ht (height, fast)}
\begin{enumerate}
\item The mktr operator must put the height in the tree it builds.
\item The new height operator, ht,
      will simply extract the height that mktr
      records in the tree (rather than computing the height).
\item The treep predicate that is used to determine whether or not
      a given entity is a tree, must account for the
      height record in the new representation.
\item The rot+ and rot- operators must refer to ht
      instead of the old, slow height operator.
\end{enumerate}
\end{quote}

Figure \ref{defun:balance-factor} (page \pageref{defun:balance-factor})
formalizes the new versions of mktr, ht, treep, and rot+.
The other operators remain unchanged,
and the only changes needed in rot+ and rot-
...
\begin{aside}
The reason the operator time-chk delivers only the height
...
After you complete the timings with the slow version of ins,
...
\begin{ExerciseList}
\Exercise Carry out the timing experiment described
