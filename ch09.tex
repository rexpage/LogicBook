...
\emph{recurrence equations}.}\\[-6mm]
\begin{center}\label{def:recurrence-equations}\index{axiom, by name!\{d1\}, \{d2\} (steps in dmx)}\index{equation, by name!\{d1\}, \{d2\} (steps in dmx)}\index{recurrence equations!dmx}\index{computation steps!dmx operation}\index{operator, by name!dmx (demultiplexer)}
\begin{tabular}{ll}
  $D_0 = D_1 = 5$      & \{d1\} \\
  $D_{n+2} = D_n + 14$ & \{d2\} \\
\end{tabular}
...
Figure~\ref{fig:dmx-computation-time} %(page \pageref{fig:dmx-computation-time})
proves this conjecture using strong induction
...
\label{fig:dmx-computation-time}\index{theorem, by name!\{dmx computation steps\}}
\end{figure}

\begin{exercises}

\exer {\label{ex:recurrence-len}%
Derive recurrence equations for the number of steps in the computation of \textsf{(len $xs$)}
from the axioms for the \textsf{len} operator (figure~\ref{fig:len-axioms}, page \pageref{fig:len-axioms}).
...
\exer {Use the recurrence equations from exercise \ref{ex:recurrence-append} to
...
but we will look for an upper bound.
...
\textsf{(mrg (rest $xs$) $ys$)} or \textsf{(mrg $xs$ (rest $ys$))}.
...
\begin{center}\index{theorem, by name!\{mrg computation steps\}}
\begin{tabular}{l}
...
\exer{The double index
of $M_{j,k}$ made the proof a little tricky.
Another approach would have been to use the principle of double induction
(figure~\ref{double-induction-rule}, page \pageref{double-induction-rule}),
...
\hspace*{3mm}a. $Q(0)$ is the base case for double induction on $P$.\\
\hspace*{3mm}b. $(\forall n.(Q(n) \rightarrow Q(n+1)))$ is the inductive case
...
\begin{center}
\begin{tabular}{ll}
\multicolumn{2}{l}{Prove  $(\forall m.P(m,0)) \wedge (\forall n.P(0,n))$                                 \hfill \emph{base case}}\\
 - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - &\\
\multicolumn{2}{l}{Prove $(\forall m.(\forall n.((P(m+1, n) \wedge P(n, m+1)) \rightarrow P(m+1,n+1))))$ \hfill \emph{inductive case}}\\
--------------------------------------------------------------------------------------\{dbl ind\}  &\\
...
\label{msort-recurrences}\index{recurrence equations!merge-sort (msort)}\index{axiom, by name!\{s1\}, \{s2\} (steps in msort)}\index{equation, by name!\{s1\}, \{s2\} (steps in msort)}\index{S recurrence (steps in msort)}
\end{figure}
...
The coincidence that 42 works may amuse \index{Adams, Douglas}Douglas Adams fans.}
...
\label{thm:msort-nlogn}\index{theorem, by name!\{msort $n~log(n)$\}}
\end{figure}

\begin{exercises}

\exer {\label{defun:Srecur}%
...
\exer {\label{defun:log2}%
For any nonzero positive number $x$, $\lfloor log_2(x)\rfloor$
is the biggest integer $n$ such that $2^n \leq x$.
Define an operator \textsf{log2} in ACL2 that computes $\lfloor log_2(x)\rfloor$
for $x \geq 1$.}

\exer {If $S_{n+2} \leq 42(n+2)\lfloor log_2(n+2)\rfloor$, then
...
\index{insertion-sort}\textsf{isort} operator
varies widely, but
...
In the end, with a little more algebraic reorganization,
...
\label{fig:isort-steps}\index{computation steps!insertion-sort (isort)}\index{recurrence equations!isort (insertion-sort)}\index{I (isort recurrence)}\index{sorting!insertion-sort (isort)}
\end{figure}
...
To compare the time it takes to sort a
...
\exer {\label{ex:insert-recurrence}%
...
is $8(n+1)+6$ or less if $x_1 \leq x_2 \leq \dots x_{n+1} < x$.}

\exer {The multiplier $42$ in our bound on $S_n$ gets sloppier as $n$ increases.
Find  $\beta < 42$ that works for lists with
over 100 elements:
$S_{n} \leq \beta\cdot n~log_2(n)$ if $n > 100$.}

\exer {\label{ex:triangular-number}%
Use induction to prove the formula for the triangular number.\\
\hspace*{1cm}$(1 + 2 + 3 + \dots n) = n(n+1)/2$\\
You may be the billionth person to prove it. Knock yourself out.}
\end{exercises}
