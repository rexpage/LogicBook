...
\begin{ExerciseList}
\Exercise
Do a pencil-and-paper proof that the \textsf{isort} operator
...
The induction scheme that it used
to prove that the \textsf{mrg} operator terminates,
namely induction on the total number of elements in the operands,
also works in the proof of the merge order theorem.
A pencil-and-paper proof could also use that strategy.
...
\Exercise
Do a pencil-and-paper proof of the mrg-length theorem
...
\Exercise
Do a pencil-and-paper proof of the mrg-val theorem of Exercise \ref{ex:mrg-val-thm}.
...
\begin{ExerciseList}
\Exercise
Do a pencil-and-paper proof that, under certain conditions, the \textsf{dmx} operator
delivers lists that are shorter than its operand
(\textsf{dmx-shortens-list-thm}, page \pageref{defthm:dmx-shortens-list}).

\Exercise
Do a pencil-and-paper proof that the \textsf{msort} operator
...
It turns out that the number of steps for \textsf{(msort $xs$)}
is proportional to $n~log(n)$, where $n$ $=$ \textsf{(len $xs$)}.
...
to the basic operators listed in the figure.
...
If $xs$ has less then two elements, then \textsf{(swap2 $xs$)} takes three steps:
...
\Exercise
Define a theorem in ACL2 about the formula \textsf{(C-from-F (F-from-C $x$))}. \\
...
\label{def:recurrence-equations} \emph{recurrence equations}.}
...
correct.\footnote{A
\label{footnote:closed-form}closed-form formula for $D_n$ is a formula
...
\begin{tabular}{lrcl}
  ...
  \hline
   ...
   \textsf{msort}  & $S_{\lfloor n/2 \rfloor}$ && \{dmx-len-second\}, page \pageref{thm:dmx-length-first-second} \vspace{1pt} \\
   \hline
...
computation steps for worst case
\textsf{(insert $x$ [$x_1$ $x_2$ \dots $x_{n+1}$])},
the average number of steps would be
...
in a poll of discrete math textbooks.
It is also Exercise~\ref{ex:triangular-number} (page \pageref{ex:triangular-number}).}
...
Use mathematical induction to prove the formula for the triangular number:
$(1 + 2 + 3 + \dots n) = n(n+1)/2$. (You may be the billionth person to have done so.)
...
$S_{n} \leq \beta\cdot n~log_2(n+2)$ if $n > 100$.
