...
\label{defun:isort}\index{equation, by name!\{isrt0\}, \{isrt1\}, \{isrt2\}}\index{operator, by name!isort (insertion-sort)}\seeonlyindex{isort}{operator}\index{insertion-sort}\index{sorting!insertion-sort (isort)}
\begin{code}
\begin{verbatim}
(defun isort (xs)
...
\exer {The \index{operator, by name!del (delete value)}
...
\label{defun:mrg}\index{equation, by name!\{mg0\}, \{mg1\}, \{mgx\}, \{mgy\}}\index{operator, by name!mrg (ordered merge)}\seeonlyindex{mrg (ordered merge)}{operator}\index{sorting!merge, ordered (mrg)}\index{merge, ordered}\index{defun!induction hint}\index{definition!defun hint}\index{directive!induction hint}
\begin{code}
\begin{verbatim}
(defun mrg (xs ys)
...
\exer {Do a paper-and-pencil proof the mrg-ord theorem (page \pageref{defthm:mrg-ord}).}
\end{exercises}
...
\end{tabular}
\end{spacing}
\end{center}\index{computation steps!one-step operators}\index{computation steps!ACL2}\index{computation steps!counting}
...
\label{fig:isort-steps}\index{computation steps!insertion-sort (isort)}\index{recurrence equations!isort (insertion-sort)}
\end{figure}
