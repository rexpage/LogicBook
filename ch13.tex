...
using two tables, one for the status update\index{update, database}updates,
...
For example, \index{Amazon}\index{store front}Amazon offers over ten million items for sale.
...
are knows as \index{Web 2.0 company}Web 2.0 companies.
...
Third, retrieval\index{retrieval, database}retrieval,
...
The reason is that the related \index{record, database}recordrecords are not necessarily stored in the same shard.
