...
$=$ & \textsf{(twos $w$ $(- n)$)}              & \{$2s-$\} (\{twos\}, page \pageref{twos-defun}) \\
\end{tabular}
%\end{center}
...
Prove theorem \{\emph{len-2s}\}:
$\forall w.((n \in I(w)) \rightarrow ($\textsf{(len (twos $w$ $n$))} $= w))$.}

\exer {\label{minus-sign}%
\index{two's-complement!sign}\index{theorem, by name!\{minus-sign\}}Prove theorem \{\emph{minus-sign}\}:
$\forall w.(((n \in I(w)) \wedge (n < 0)) \rightarrow ($\textsf{(fin(twos $w$ $n$))} $=$ $1))$.\\
The operator \textsf{fin} is defined on page \pageref{fin-defun}.}

\exer {\label{plus-sign}%
\index{two's-complement!sign}\index{theorem, by name!\{plus-sign\}}Prove theorem \{\emph{plus-sign}\}:
...
\index{circuit!two's-complement negation}Diagram a negation circuit whose input signals represent a
...
(figure~\ref{fig:2s-comp-negation}),
and draw a box labeled ``adder''
to depict a ripple-carry adder circuit
(figure \ref{fig:adder-thm}, page \pageref{fig:adder-thm},
depicts an adder circuit using such a box).\\
...
exercise~\ref{2s-negation-circuit-model} when $w = 32$.
You may refer to the operator \textsf{expt}: \textsf{(expt 2 31)} $=$ $2^{31}$.}

\exer {\label{2s-subtraction-circuit}%
\index{numeral!subtraction}\index{subtraction, binary numeral}\index{circuit!subtraction}Diagram
a circuit that subtracts two's-complement numerals.
...
You may refer to the operator \textsf{expt}: \textsf{(expt 2 ($-$ $w$ 1))} $=$ $2^{w - 1}$.}

\exer {\label{ex:geometric-progression}%
Use induction to prove the following equation. You may assume $r > 0$ and $r \neq 1$.\\
