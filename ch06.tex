...
(defun twos (w n)             ; w = word size
  (if (< n 0)                 ; -2^(w-1) <= n < 2^(w-1)
      ...
      (pad w 0 (bits n))))    ; {2s+}
\end{verbatim}
\end{code}\index{two's-complement!operator}\index{operator, by name!twos (two's-complment)}\index{operator!two's-complement}\index{equation, by name!\{2s$+$\}, \{2s$-$\}}\index{arithmetic!negative numeral}
