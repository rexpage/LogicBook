...
that generates 
\index{random data!generating}\index{random data!testing with}\index{testing!with random data}random data
...
\index{testing!with random data}\index{random data!testing with}\index{defproperty}\textsf{defproperty} 
...
\begin{aside}
A \index{natural number}\emph{natural number} is a whole number 
...
\begin{aside}
\index{arithmetic!modular}\index{modular arithmetic}\index{division!floor, mod}\index{operator, by name!mod (remainder)}Modular 
arithmetic
...
Generating 
\index{random data!generating}\index{testing!with random data}random data 
is an art.
...
\index{let* names for values}\index{formula!let*}\index{variable!let*}\textsf{let$*$} formula,
...
\index{formula!let$*$}\index{variable!let$*$}\index{let$*$ names for values}\index{value, let$*$ name for}\textsf{let$*$}, 
after the name-value bindings.
\caption{Naming Local Values with \textsf{let$*$}}
\label{let*-def}
\end{aside}

Another property of the \textsf{mod} operator
...
\index{compare!numbers ($<$, $<=$, $=$, $>=$, $>$)}\index{predicate!numeric order ($<$, $<=$, $=$, $>=$, $>$)}\index{order!numeric ($<$, $<=$, $=$, $>=$, $>$)}\index{operator!numeric order ($<$, $<=$, $=$, $>=$, $>$)}\index{number!order ($<$, $<=$, $=$, $>=$, $>$)}\seeonlyindex{less than ($<$)}{predicate}
\begin{Verbatim}
