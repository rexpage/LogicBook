...
\index{operator, by name!cons (insert at front)}operator \textsf{cons}
...
The formula \textsf{[8 3 7]} is pencil-and-paper shorthand 
...
The universe of discourse \textsf{consp} is the set of lists that ACL2 can represent.
...
\index{(first nil)}(first nil) and \index{(rest nil)}\textsf{(rest nil)},
...
For example, the operator \textsf{len} (page \pageref{len-op-informal})
...
have one more element than it would have if
...
\begin{ExerciseList}
\Exercise \label{rst1}
Prove \{\emph{rst1}\}: \textsf{(rest [$x$])} $=$ \textsf{nil}.\\
...
The operator \textsf{append}, previously described informally with \textsf{check-expect} tests
...
One way to provide a more formal definition of \textsf{append} is
...
that value is \textsf{(append (rest $xs$) $ys$)}.
However, if $xs$ is nil, the value of append
...
uses the \textsf{consp} and \textsf{if} operators to select between them.
...
cite some axioms about \textsf{append}, \textsf{len}, and some axioms from numeric algebra,
...
\begin{aside}
Inductive definitions are one of many ways to specify software,
...
\index{why inductive?}\index{definition!why inductive?}\index{inductive, why?}
...
\hspace*{16mm}$\forall n$ \emph{universe of discourse: natural numbers};
$\forall x$ \emph{universe of discourse: numbers}\\
...
which has four parts.\index{defun}\index{definition!defun}\index{definition!operator}
\begin{quote}
\textsf{(defun $f$ ($x_1$ $x_2$ $\dots$ $x_n$)} \emph{$\dots$ACL2-formula$\dots$})
\begin{enumerate}
\item the keyword \textsf{defun}
\item a name for the operator being defined ($f$)
\item a list enclosed in parentheses of names designating operands \textsf{($x_1$ $x_2$ $\dots$ $x_n$)}
...
The predicate \textsf{zp} imposes the same constraint to natural numbers,
...
\Exercise\label{member-equal-equations}\index{axiom!member-equal}\index{equation!member-equal}\seeonlyindex{member-equal}{predicate}\index{operator, by name!member-equal (\emph{see} predicate)}\index{predicate, by name!member-equal (element of list)}\index{axiom, by name!\{mem0\}, \{mem1\}}\index{equation, by name!\{mem0\}, \{mem1\}}
