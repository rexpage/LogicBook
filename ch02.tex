...
again.\index{panic, don't}\index{disheartened?}\index{frustration}\index{baffled?}\index{worried?}\index{scared?}\index{funk, in a}
...
equals for equals applies the first of Euclid's common notions:
...
In our experience,
\index{copy-and-paste programming}copy-and-paste programming is
...
\index{absorption}\index{equation!absortion}\index{equation, by name!\{$\wedge$ absorption\}}\{$\wedge$-absorption\}
theorem
(Figure~\ref{and-absorption-thm}, page \pageref{and-absorption-thm}),
...
\index{exclusive or}\index{or, exclusive}\emph{Note}:
These formulas define the exclusive-or operator.
...
However, it is good to have a precise definition of the grammar.
We start with the most basic elements,
then up to more complicated ones.
...
\end{tabular}
\end{center}
\index{Boolean!algebra}\index{formula!grammar}\index{grammar, of formulas}
...
\index{equation!grammar}\index{formula!grammar}\index{grammar, of formulas}
\begin{tabular}{cc}
\hline
\emph{symbol from \{bin-op\} rule}      & \emph{matching element in}  $(x \vee (\neg y))$ \\
\hline
...
\begin{aside}
The following equation defines the Boolean equivalence operator ($\leftrightarrow$).
\vspace{1mm}
\begin{center}
...
Boolean equivalence.\index{Boolean!equivalence ($\leftrightarrow$, iff)}\index{operator!Boolean equivalence ($\leftrightarrow$)}\index{equivalence!Boolean ($\leftrightarrow$)}
...
There are logic
\index{digital gate}\index{Boolean!gate}\index{gate!logic}\index{logic!gate}gates
...
\index{gate!functionally complete}\index{operator!functionally complete}\index{gate!universal}\index{universal gate}\index{functionally complete}\emph{functionally complete} (or \emph{universal})
...
but their negations,
\index{nor!logic gate}\emph{nor} and \index{nand!logic gate}\emph{nand}, are.
...
and deliver only two distinct \index{signals, Boolean}\index{Boolean!signals}signals on an output line.
...
at the same time, since equations go both ways. Deductive \index{reasoning!deductive}\index{deductive reasoning}reasoning is one-directional.
...
which is called a \index{turnstile}turnstile,
to separate the \index{hypothesis}hypotheses from the \index{conclusion}conclusions.
...
The formulas on the left are the \index{hypothesis}hypotheses of the theorem, and
the one on the right is the \index{conclusion}conclusion.
...
\index{discharge  hypothesis}\index{hypothesis!discharge}
\index{inference rule!discharge hypothesis}
...
to be a logical-and formula $(a \wedge b)$.
Some rules place constraints on the formulas
...
The \{$\wedge$ elimination 1\} rule brings the left-hand operand of the $\wedge$
down below the line.($a \rightarrow False$)
...
\index{inference rule, by name!\{identity\}}
\index{inference rule, by name!\{$\rightarrow$ introduction\}}
\index{hypothesis!discharge}\index{discharge hypothesis}\index{Assume (natural deduction)!discharge}
\index{inference rule!discharge hypothesis}
...
\index{hypothesis!discharge}\index{discharge hypothesis}\index{Assume (natural deduction)!discharge}\index{inference rule!discharge hypothesis}\index{proof!discharge hypothesis}\index{Assume (natural deduction)!discharge}discharging
...
However, a
\index{hypothesis!discharge}\index{discharge hypothesis}\index{Assume (natural deduction)!discharge}\index{inference rule!discharge hypothesis}\index{proof!discharge hypothesis}discharged
assumption
...
\index{implication}$(a \rightarrow a)$ has the value $True$, regardless of
...
\index{inference rule, by name!\{$\vee$ introduction 1\}}
\index{inference rule!discharge hypothesis}
\index{discharge hypothesis}\index{Assume (natural deduction)!discharge}
\index{hypothesis!discharge}
\index{theorem, by name!\{$\vee$ commutes\}}
\index{commutative}
...
\index{bound variable}\index{variable!bound}``bound variable''
...
called a
\index{bound variable}\index{variable!free}\index{variable!bound}\index{free variable}\label{def:free-variable}``free variable''.
...
That is, there is a value (namely, the number 5) in the domain
...
so it can be proved using the same methods we used to prove Boolean
formulas.
...
is a natural number $n$ for which $Odd(n$) is false (the number $2$, for example).
...
are false, the truth table for $\vee$ shows that the formula is false.
...
Furthermore, the formula can have the value $True$,
depending on what number $x$ stands for,
...
However, we must be careful not to change any occurrence of a different bound variable
...
Next, we rename the bound variable \emph{x} in $(\dots(\forall x\dots)\dots)$
...
Migrating the quantifiers is a tedious process and requires
...
\index{quantifier!negation}
\index{universal quantifier ($\forall$)!negation}
\index{quantifier!universal ($\forall$)}
\index{forall ($\forall$)!negation}
\index{existential quantifier ($\exists$)!negation}
\index{there exists ($\exists$)!negation}
