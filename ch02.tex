...
\hspace*{1cm}$(x \leftrightarrow y) = ((x \rightarrow y) \wedge (y \rightarrow x))$
\vspace{2mm}\\
If an implication goes both ways $(x \leftrightarrow y)$, 
then $x$ and $y$ have the same value ($x = y$).
This is the negation of the statement that 
$x$ and $y$ have different values (one is $True$, the other $False$),
which is the exclusive-or operation defined by the formula
$(x \wedge (\neg y)) \vee ((\neg x) \wedge y)$.
Exercise\ref{ex:boolean-equiv-notxor} shows that Boolean equivalence
is the negation of the exclusive-or operator.
Exercises~\ref{ex:boolean-equiv1}, \ref{ex:boolean-equiv2}, and \ref{ex:boolean-equiv4}
show three other formulas that also produce 
the same value as the Boolean equivalence operator. 
In other words, the four formulas on the right-hand sides of the equations
in those exercises are all equivalent to 
