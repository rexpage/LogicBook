...
formulas.
So, the \label{defnvariable}\index{variable}variable $x$ stands for a grammatically correct formula,
which could be something simple, such as $2$, or
it could be a more complicated formula, such as $3 \times (y + 1)$.
(Some treatments call $x$ a
\index{variable!metavariable}\index{metavariable}metavariable in this context.)
...
\end{tabular}
\end{spacing}
\end{center}\index{equation, by name!\{$\times$ comm3\}@\{$+$ identity\}}\index{equation, by name!\{$\times$ comm2\}@\{$+$ complement\}}\index{equation, by name!\{$\times$ identity\}}\index{equation, by name!\{$\times$ null\}}\index{equation, by name!\{$\times$ comm\}@\{$+$ commutative\}}\index{equation, by name!\{$\times$ commutative\}}\index{equation, by name!\{$\times$ assoc\}@\{$+$ associative\}}\index{equation!numeric algebra}\index{axiom!numeric algebra}\index{reasoning!with equations}
...
\label{or-truth-table}\index{equation!proof by}\index{proof!with equations}\index{truth table}\index{theorem, by name!\{$\vee$ truth table\}}
\end{figure}
...
and the proof of that equation simply amounts to making that observation.
...
is no learning. $\dots$ \emph{We'll wait here.}
...
\index{equation, by name!\{$\vee$ complement\}}\index{theorem, by name!\{$\vee$ complement\}}
...
\index{equation!proof by}
\index{proof!with equations}
\index{truth table}\index{theorem, by name!\{$\neg$ truth table\}}
...
\index{truth table}\index{theorem, by name!\{$\rightarrow$ truth table\}}
...
\index{proof!with equations}\index{theorem, by name!\{$\wedge$ null\}}
...
It can have hundreds of variables and thousands of operations.
...
$=$ & $y \vee (x \wedge False)$            & \{$\vee$ distributive\} \\
...
\index{equation, by name!\{$\wedge$ absorption\}}\index{theorem, by name!\{$\wedge$ absorption\}}
...
(figure~\ref{and-absorption-thm}, page \pageref{and-absorption-thm})
to derive the
\index{theorem, by name!\{$\vee$ absorption\}}\index{equation, by name!\{$\vee$ absorption\}}\{$\vee$ absorption\} equation: $(x \wedge y) \vee y = y$.}
...
\index{axiom, by name!\{redundant grouping\}}
...
\index{equation, by name!\{redundant grouping\}}
...
\end{spacing}
\end{center}
\index{equation, by name!\{$\vee$ abs1\}@\{$\neg$ as $\rightarrow$\}}
\index{equation, by name!\{$\wedge$ deMorgan\}}
\index{equation!DeMorgan}
\index{DeMorgan}
\index{equation, by name!\{$\vee$ complement\}}
\index{equation, by name!\{$\wedge$ complement\}}
\index{equation, by name!\{$\vee$ abs3\}@\{$\neg True$\}}
\index{equation, by name!\{$\vee$ abs2\}@\{$\neg False$\}}
\index{equation, by name!\{$\wedge$ null\}2@\{$\rightarrow$ identity\}}
...
\index{theorem, by name!\{$\wedge$ deMorgan\}}
\index{theorem, by name!\{$\vee$ complement\}}
\index{theorem, by name!\{$\wedge$ complement\}}
\index{theorem, by name!\{$\neg False$\}, \{$\neg True$\}}
\index{theorem, by name!\{$\rightarrow$ identity\}}
\index{theorem, by name!\{$\wedge$ identity\}}
\index{theorem, by name!\{$\wedge$ commutative\}}
\index{theorem, by name!\{$\wedge$ associative\}}
\index{theorem, by name!\{$\wedge$ distributive\}}
\index{theorem, by name!\{$\wedge$ idempotent\}}
\index{theorem, by name!\{idempotent\}}
\index{theorem, by name!\{contrapositive\}}
\index{theorem, by name!\{currying\}}
\index{theorem, by name!\{$\vee$ absorption\}}
\index{theorem, by name!\{$\wedge$ implication\}}
\index{theorem, by name!\{$\wedge$ implication\}}
\index{theorem, by name!\{absurdity\}}
\index{theorem, by name!\{contradiction\}}
...
\index{operator, logic!Boolean equivalence ($\leftrightarrow$, iff)}Boolean equivalence operator ($\leftrightarrow$):
\begin{center}
...
Boolean equivalence.\index{Boolean!equivalence ($\leftrightarrow$, iff)}\index{equivalence!Boolean ($\leftrightarrow$, iff)}
...
and the lack of voltage, the signal 0 (\emph{False}),
...
represent the same concepts in logic.
\index{circuit!diagram}\index{diagram!circuit}Circuit diagrams,
logic formulas,
...
conclusion of the first proof above the line to
...
stand for formulas (see page \pageref{defnvariable}).
Any grammatically correct formula
can be plugged in for a variable
(sometimes called a \index{metavariable}metavariable in this context),
as long as that same formula replaces all instances of the variable.
For example,
theorem \{$\wedge$ commutes\} ($a \wedge b$ $\vdash$ $b \wedge a$)
...
of the theorems we've proved so far using natural deduction
...
but one. The rule we haven't used yet is \{contradiction\}.\footnote{It
...
corresponding to the citation of the \{contradiction\} rule in the second proof.
...
---------------\{$\vee$ elimination\}       &\emph{requires three proofs above the line, scope goes to top}\\
...
\{nor commutes\}: $\neg (a \vee b)$ $\vdash$ $\neg (b \vee a)$\\
\emph{Note}: The \{$\vee$ commutes\} theorem may help, but not directly
...
Use natural deduction to prove
\index{DeMorgan}\index{theorem!DeMorgan}\index{theorem, by name!\{deMorgan $\vee$ forward\}@\{DeMorgan $\vee$ forward\}}\{DeMorgan $\vee$ forward\}:
$\neg (a \vee b)$ $\vdash$ $(\neg a) \wedge (\neg b)$}

\exer {\label{ex:DeMbkw}%
Use natural deduction to prove
\index{DeMorgan}\index{theorem!DeMorgan}\index{theorem, by name!\{deMorgan $\vee$ backward\}@\{DeMorgan $\vee$ backward\}}\{DeMorgan $\vee$ backward\}:
$(\neg a) \wedge (\neg b)$ $\vdash$ $\neg (a \vee b)$}
...
cite the \{nor elimination 1\}
theorem from exercise~\ref{ex:nor-elimination},
and remember that a particular assumption can
occur at multiple points in a proof.}
...
In the formula $(\forall x.P(x)) \vee Q(y)$,
...
each formula has:
\subexer {$(P(x) \vee (P(y) \rightarrow P(z)))$}
\subexer {$(\forall x.(P(x) \wedge (\forall y.Q(y))))$}
\subexer {$(P(x) \rightarrow (\exists y.Q(y)))$}
...
the name of a bound variable has no effect on the value
of the formula:
...
\index{equation, by name!\{$\forall\wedge\rightarrow$\}@\{${\forall}{\rightarrow}$\}}
\index{equation, by name!\{$\exists\wedge\rightarrow$\}@\{${\exists}{\rightarrow}$\}}
\index{equation, by name!\{$\forall\wedge\rightarrow$\}2@\{${\rightarrow}{\forall}$\}}
\index{equation, by name!\{$\exists\wedge\rightarrow$\}2@\{${\rightarrow}{\exists}$\}}
\index{equation, by name!\{$\forall\wedge\rightarrow$\}3@\{R$\forall$\}}
\index{equation, by name!\{$\exists\wedge\rightarrow$\}2@\{R$\exists$\}}
...
That explains the note in
figure~\ref{fig-02-quantifiers} (page \pageref{fig-02-quantifiers})
prohibiting the occurrence of $x$ as
...
rules $\{\neg\forall\}$ and $\{\neg\exists\}$
are quantifier analogs of the DeMorgan equations
(figures~\ref{fig-02-boolean-axioms} and \ref{some-boolean-theorems},
pages \pageref{fig-02-boolean-axioms} and \pageref{some-boolean-theorems}).
...
\exer {Prove equation \{$\exists\wedge$\}.}
...
assertions about the real world stated in the form of true/false propositions.
...
We will try to simulate the play of the game of Nim using propositional logic.
...
Another approach is to let $x10$ mean ``there are 10 stones in the pile,''
...
For example, if both $x1$ and $x5$ were $True$,
...
times 11 move numbers (move 0 through move 10,
...
the legal ways in which a pile transforms from one move to the next\\
\end{tabular}
\end{center}
...
Since Alice moves first, her move numbers are odd (1, 3, 5, \dots),
and Bob's are even (2, 4, 6, \dots).
...
for a combination of values of the variables that represents a particular game,
...
The legal transitions correspond to the actual program and how it manipulates information over time.
And a traditional way to specify that the program is finished
...
In any game of Nim, there is a time at the end when no stones remain in the pile,
so the formula  $(\exists t.X(0,t))$
is $True$.
...
\item the legal ways in which a pile transforms from one step to the next, AND
...
Consider the formula with the meaning ``Alice wins.''
