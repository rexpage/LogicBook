...
\index{seat, hold on}\emph{Hold on to your seat.}
...
negation gate (also known as an \index{inverter, gate}\index{gate!inverter}inverter).
...
which is called a \index{turnstile ($\vdash$)}turnstile,
...
where the domain of discourse for both quantifiers is the integers.\footnote{$Odd(x) \equiv (\exists y.(x = 2y+1))$.
\label{even-number-predicate-Even}
$Even(x) \equiv (\exists y.(x = 2y))$.
The integers are the universe of discourse 
for the predicates $Odd$ and $Even$ (variable $x$)
and the quantifications (bound variable $y$).}
...
take care to see how each equation we cite justifies
the migration of a quantifier in the formula.

The equations of Figure~\ref{fig-02-quantifiers} show how quantifiers
interact with certain logic operators ($\wedge$, $\vee$, $\rightarrow$, and $\neg$).
In the equations with binary operators, one of the operands
is a proposition $Q$ that stands outside the quantification
on the left-hand side of the equation, but inside
the quantification on the right-hand side of the equation.
Since $x$ is the bound variable in the quantification,
the formula for $Q$ must not have a free variable named $x$.
If it did, that variable would be \emph{captured} by the quantification
thereby becoming a bound variable, which would change the meaning
of the formula. This explains the note in the figure
that constrains variables in $Q$. 
A similar constraint applies to the renaming
equations (\{R$\forall$\} and \{R$\exists$\}),
and for the same reason: to avoid the 
\index{capture, variable}\index{bound variable!capture}\index{variable!capture}\index{free variable!capture}capture 
of a variable by a quantification.
The constraint doesn't affect the applicability of
the equations because the renaming equations
can always be used to change the names of bound variables
to avoid conflicts.
...
show how quantifiers behave interact with implication ($\rightarrow$).
...
on the right-hand side of the equation. When the quantification is on
the left-hand-side of the implication, as it is in
equations (\{$\forall\rightarrow$\} and \{$\exists\rightarrow$\}),
the quantified formula, $P(\dots x, \dots)$, becomes the
left-hand operand of the implication.
That change flips the sense of the quantification.
It becomes $\forall$ if it was $\exists$ and
$\exists$ if it was $\forall$.\footnote{This
flip is related the \{implication\} equation,
($(x \rightarrow y) = ((\neg x) \vee y)$,
Figure~\ref{fig-02-boolean-axioms}, page \pageref{fig-02-boolean-axioms})
and the quantifier negation equations (\{$\neg\forall$\} and \{$\neg\exists$\}).}
The flip does not occur when the quantification 
is on the right-hand side of the implication
because in that case, the quantified formula, $P(\dots x, \dots)$, becomes the
right-hand operand of the implication.
...
The $\exists$ quantification is not particular about
...
With these choices,
the formula of interest is $(P(x_0, v_0) \rightarrow P(x_0, v_0))$.
