...
\begin{center}
  \index{axiom, by name!\{lst\}}Axiom \{\emph{lst}\}: $(\forall xs.(\exists n.(\exists$\textsf{[$x_1$ $x_2$ $\dots$ $x_n$]}$.(xs =
...
Try it for yourself in a Proof Pad session.
Enter the \textsf{include-book} directive
...
\exer {Define the
\index{append!associative}\index{theorem!append associative}\index{theorem, by name!\{app-assoc\}}\index{associative}associativity property
...
it possible for ACL2 to prove termination is, by itself,
...
\exer {Define the \index{theorem, by name!\{rep-len\}}\{\emph{rep-len}\} theorem (page \pageref{rep-len}) in ACL2,
...
\exer {Exercise~\ref{ex:nthcdrtonil} on page \pageref{ex:nthcdrtonil}
...
\end{tabular}
\end{center}
\index{Fibonacci!numbers}\index{axiom!Fibonacci}\index{operator, by name!fib, fib-fast (Fibonacci)}\index{Fibonacci!operator}\index{equation, by name!\{f0\}, \{f1\}, \{f2\} (Fibonacci)}\index{equation, by name!\{fib1\}, \{fib2\}}\index{axiom, by name!\{fib1\}, \{fib2\}}\index{axiom, by name!\{f0\}, \{f1\}, \{f2\} (Fibonacci)}
...
It's not too hard to see the reason for this.
...
\end{code}
\end{center}
\seeonlyindex{fib, fib-fast}{operator}\index{Fibonacci!fast}\index{Fibonacci!operator}\index{operator, by name!fib, fib-fast (Fibonacci)}\index{operator, by name!gib (iterative Fibonacci)}\index{axiom, by name!\{gib0\}, \{gib1\}, \{gib2\} (Fibonacci)}\index{equation, by name!\{gib0\}, \{gib1\}, \{gib2\} (Fibonacci)}\index{gib (\emph{see also} operator)}
...
The theorem in ACL2 terms can be stated
\index{theorem, by name!\{fib=fib-fast\}}\index{Fibonacci!fast}\index{theorem!Fibonacci fast}as follows:
\begin{center}
\begin{code}
\begin{verbatim}
...
\begin{center}\index{gib (\emph{see also} operator)}\index{theorem!gib lemmas (base, inductive)}\index{gib (\emph{see also} operator)!lemmas (base, inductive)}\index{theorem, by name!\{gib-inductive-equation\}}
\begin{code}
\begin{verbatim}
...
\index{theorem!gib lemmas (base, inductive)}\index{gib (\emph{see also} operator)!lemmas (base, inductive)}\index{theorem, by name!\{gib-base-equation\}}
\begin{center}
\begin{code}
\begin{verbatim}
...
by \index{turing alan@Turing, Alan}Alan Turing in 1936
and, independently, by \index{church alonzo@Church, Alonzo}Alonzo Church.
...
\label{church-turing-hypothesis}\index{church turing hypothesis@Church--Turing hypothesis}\index{turing church hypothesis@Turing--Church hypothesis}\index{hypothesis!church turing@Church--Turing}A
...
\begin{center}
\begin{tabular}{ll}
Assume $(\neg(\neg E))$                       &\emph{this assumption will be discharged}\\
...
\index{theorem, by name!\{hf\}@\{hF\}, \{hT\}}
\vspace{2mm}\\
%\begin{quote}
...
\label{fig:paradox-op-defun}\seeonlyindex{loop}{operator}\index{operator, by name!loop}\seeonlyindex{p (paradox)}{operator}\index{operator, by name!p (paradox)}\index{axiom, by name!\{p0\}, \{p1\}}\index{equation, by name!\{p0\}, \{p1\}}
\end{figure}
...
\index{theorem, by name!\{pf\}@\{pF\}, \{pT\}}
\vspace{2mm}\\
%\begin{quote}
...
That is, we interpret the substitution of a new formula
...
\vspace{2mm}\\
%\begin{quote}
\hspace*{5mm}\index{theorem, by name!\{h0\}, \{h1\}}Theorem \{h0\}: $\vdash$  \textsf{(h p $x$)} $=$ \textsf{0}  $\rightarrow$ \textsf{(p $x$)} $=$ \textsf{0}    \\
...
\vspace{2mm}\\
%\begin{quote}
\hspace*{5mm}\index{theorem, by name!\{p0\}, \{p1\}}Theorem \{p0\}: $\vdash$  $($\textsf{(p $x$)} $=$ \textsf{0}$)$ $\rightarrow$ $H($\textsf{p}, $x)$ \\
...
%\begin{quote}
\label{thm:HplusHminus}\index{theorem, by name!\{h$+$\}@\{H$+$\}, \{H$-$\}}\hspace*{2mm}Theorem \{H$+$ version 2\}: $E$ $\vdash$ $H($\textsf{p}, $x)$
...
~~~~~Assume $E$                                 &\emph{hypothesis of theorem}\\
------------------------\{H$-$ version 2\}      &\{H$-$\}\emph{ proved in figure~\ref{fig:hminus-thm-proof} (page \pageref{fig:hminus-thm-proof})}\\
~~~~~~~~$\neg H($\textsf{p}, $x)$               &\\
...
\index{operator, by name!loop}\textsf{(loop $x$}) represents
...
%\begin{quote}
\hspace*{19mm}$\forall n.\forall x.($\textsf{(loop $x$)} $=$ \textsf{(loop$^{n+1}$ $x$)}$)$
%\end{quote}
}
