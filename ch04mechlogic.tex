...
\Exercise Define the
\index{append!associative}\index{theorem!append associative}\index{theorem, by name!\{append associative\}}\index{associative}\{\emph{append associativity}\}
...
\emph{Hint}: \textsf{(defthm +associative (= (+ $x$ (+ $y$ $z$)) (+ (+ $x$ $y$) $z$)))}
...
An inspection of the definition of \textsf{p}
(Figure~\ref{fig:paradox-op-defun}, page \pageref{fig:paradox-op-defun})
confirms that (1)~\textsf{(p $x$)} $=$ \textsf{0} 
implies that \textsf{(p $x$)} terminates and
(2)~\textsf{(p $x$)} $=$ \textsf{(loop $x$)} 
implies that \textsf{(p $x$)} does not terminate.
Combining these observations with
the definition of $H$ again (page \pageref{def:predicate-H}),
proves the following two theorems, \{p0\} and \{p1\}.
\begin{quote}
