...
Acquiring a deep expertise with such tools
...
Theorem statements in ACL2 start with the ``defthm'' keyword.
...
and the ACL2 system makes them available in the form of ``certified books''
...
to J Moore, a pioneer in mechanized logic and a principal developer of ACL2,
...
The theorem is true, however, when the $xs$ variable is
...
The Fibonacci axioms comprise an inductive definition.
...
that occurs at the ``top level'', which means that it is not nested
...
The downside is that reasoning about tail recursive definitions
is often more challenging than reasoning about definitions
