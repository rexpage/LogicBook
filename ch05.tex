...
\label{fin-defun}\index{operator, by name!fin (extract last element)}\seeonlyindex{fin}{operator}\index{axiom, by name!\{fin1\}, \{fin2\}}\index{equation, by name!\{fin1\}, \{fin2\}}
\begin{quote}
\begin{code}
\begin{verbatim}
(defun fin (xs)
...
to constrain the theorem to nonzero, natural numbers.
...
\exer {\label{len-bits}%
\index{theorem, by name!\{len-bits\}, \{len-bits$\le$\}}Prove theorem \{\emph{len-bits}\}.
$\forall n.((\lfloor 2^{n-1}\rfloor \le m < 2^n) \rightarrow ($\textsf{(len (bits $m$))} $=$ $n))$~~~\{\emph{len-bits}\}}

\exer {\label{len-bitsLE}%
\index{theorem, by name!\{len-bits\}, \{len-bits$\le$\}}Prove theorem \{\emph{len-bits}$\le$\}.
...
$\forall n.($\textsf{(len (bits $(n+1)$))} $=$ $\lfloor log_2(n+1) \rfloor + 1)$~~~\{\emph{log-bits}\} \\
\emph{Note}: $\lfloor x \rfloor$ is the greatest integer not exceeding $x$.}

\exer {\label{leading-0}%
\index{theorem, by name!\{leading-0\}, \{leading-0s\}}Prove theorem \{\emph{leading-0}\}.\\
\hspace*{1cm}$\forall n.($\textsf{(numb (append (bits $n$) (list 0)))} $=$ \textsf{(numb (bits $n$))}$)$~~~\{\emph{leading-0}\}}

\exer {\label{leading-0s}%
\index{theorem, by name!\{leading-0\}, \{leading-0s\}}\index{leading zero}\index{numeral!leading zero}Assume
