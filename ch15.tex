...
we can use a list of rows, each of which is a list of $N$
...
Figure~\ref{fig:glider-in-images} (page \pageref{fig:glider-in-images})
illustrates this idea with a $4\times4$ grid of squares
...
When the entry is a 0, the pixel is ``off'' (indicated by a white square).
...
%\end{quote}
In this representation, triples of 0/1 values
specify the off/on status of color dots in the corresponding pixel.
...
whose intensities reflect the numbers in the matrix.
...
that paints a line on an image.

The \textsf{line} operator would figure out
...
and $color$ is a red-green-blue triple.
...
Today, many decades later, most of the line-drawing in digital images still
...
\begin{center}\index{axiom, by name!\{r0\}, \{r1\}}\index{equation, by name!\{r0\}, \{r1\}}\index{operator, by name!r (pseudo-random)}\seeonlyindex{r (pseudo-random)}{operator}
\begin{tabular}{ll}
...
of fascinating ramifications. It happens that the operator
...
\begin{center}\index{axiom, by name!\{rs\}}\index{equation, by name!\{rs\}}\index{operator, by name!rs (pseudo-random sequence)}\seeonlyindex{rs (pseudo-random sequence)}{operator}
\begin{tabular}{ll}
...
\label{fig:ragged-defun}\index{operator, by name!ragged (line)}\index{axiom, by name!\{rg0\}, \{rg1\}}\index{equation, by name!\{rg0\}, \{rg1\}}\seeonlyindex{ragged (line)}{operator}
\end{figure}
